        ...
        if (!picked) {
          ...
          try {
            ...
            if (!pickedEntry?.bookmark && pickedGroup && pickedGroup === targetGroup) {
              // Within-group non-tip move: just rebase, bookmark stays on tip
              fs.appendFileSync('/tmp/vpr-debug.log', `ACTION: within-group rebase -r ${picked} ${rebaseFlag} ${targetChangeId}\n`);
